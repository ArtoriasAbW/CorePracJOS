...
static bool ata_callback_pci(pci_func *device) {
...
bool ata_init(pci_func *pciDevice) {
    ...
    ata_device_t *ataDevice = (ata_device_t*)test_alloc(sizeof(ata_device_t));
    ...
    if ((pi & ATA_PCI_PIF_PRI_NATIVE_MODE) && pciDevice->BaseAddresses[0].PortMapped && pciDevice->BaseAddresses[0].BaseAddress != 0
        ...
        ataDevice->Primary.Interrupt = pciDevice->irq_line;
    }
    else {
    ...
    if ((pi & ATA_PCI_PIF_SEC_NATIVE_MODE) && pciDevice->BaseAddresses[2].PortMapped && pciDevice->BaseAddresses[2].BaseAddress != 0
        ...
        ataDevice->Secondary.Interrupt = pciDevice->irq_line;
    }
    else {
        // Use default ISA ports.
